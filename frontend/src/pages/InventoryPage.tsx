...
import { AlertTriangle, Edit, Minus, Plus, Search } from "lucide-react";
...
export function InventoryPage({ materials, currentRole, onChanged, onToast }: InventoryPageProps) {
  ...
  const lowStockCount = useMemo(
    () => materials.filter((m) => numeric(m.quantity_in_stock) <= numeric(m.min_quantity)).length,
    [materials]
  );
  ...
  const filteredMaterials = useMemo(() => {
    ...
    if (lowStockOnly) {
      result = result.filter((m) => numeric(m.quantity_in_stock) <= numeric(m.min_quantity));
    }
    const term = search.trim().toLowerCase();
    if (term) {
      result = result.filter((m) => [m.name, m.sku].join(" ").toLowerCase().includes(term));
    }
    return result;
  }, [materials, search, lowStockOnly]);

  const { page, setPage, pages, pageSize, total, visible: visibleMaterials } = usePagination(filteredMaterials);
  ...
  async function saveMaterial(event: FormEvent) {
    ...
    try {
      ...
      setModalOpen(false);
      await onChanged(editing ? "Material atualizado." : "Material cadastrado.", {
        id: `${Date.now()}`,
        action: editing ? "Atualização de material" : "Cadastro de material",
        module: "Estoque",
        target: material.sku,
        created_at: new Date().toISOString()
      });
    } catch (err) {
      onToast(err instanceof Error ? err.message : "Não foi possível salvar o material.");
    }
  }
  ...
  async function moveStock(material: Material, movementType: "in" | "out") {
    try {
      await api.inventory.createMovement({
        ...
        quantity: 1,
        notes: movementType === "in" ? "Entrada rápida via painel" : "Saída rápida via painel"
      });
      await onChanged(movementType === "in" ? "Entrada de estoque registrada." : "Saída de estoque registrada.", {
        id: `${Date.now()}`,
        action: movementType === "in" ? "Entrada de estoque" : "Saída de estoque",
        module: "Estoque",
        target: material.sku,
        created_at: new Date().toISOString()
      });
    ...
    <section className="page-stack">
      <div className="toolbar">
        <div className="toolbar-group">
          ...
          <button
            className={`button ${lowStockOnly ? "button-warning" : "button-outline"}`}
            type="button"
            onClick={() => setLowStockOnly(!lowStockOnly)}
          >
            <AlertTriangle size={17} />
            Abaixo do mínimo
            {lowStockCount > 0 ? <span className="tab-count">{lowStockCount}</span> : null}
          </button>
        </div>
        {allowedToWrite ? (
      ...
      <Table columns={["Cód.", "Descrição", "Qtd. atual", "Mínimo", "Preço unitário", "Status", "Ações"]} empty={!visibleMaterials.length}>
          ...
          return (
            <tr key={material.id} className={lowStock ? "low-stock-row" : ""}>
              ...
              <td>
                ...
                {lowStock ? <small>Estoque abaixo do mínimo</small> : null}
              </td>
      ...
      <Modal title={editing ? `Editar ${editing.sku}` : "Cadastrar material"} open={modalOpen} onClose={() => setModalOpen(false)}>
        <form className="form-grid" onSubmit={saveMaterial}>
          ...
          <label className="field">
            ...
            />
            <small>Estoque inicial é definido no cadastro. Use as setas da tabela para registrar movimentações.</small>
          </label>
          <label className="field">
